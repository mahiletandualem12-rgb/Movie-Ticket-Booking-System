...
void bookSeat(int(*seatPtr)[20], int movieIndex, int seatIndex){
...
void cancelSeat(int(*seatPtr)[20], int movieIndex, int seatIndex){
...
void updateBooking(int(*seatPtr)[20], int movieIndex){
int cancelCount, seatNo;
   ...
   for(int i =0; i< cancelCount; i++){
    ...
    int idx = seatNo - 1;
    if(idx >= 0 && idx < 20 && seatPtr[movieIndex][idx] == 0)
    {
   ...
   for(int i = 0; i< newCount; i++){
    ...
    // convert to 0 based index for internal use
    int idx = seatNo - 1;
    if(idx >= 0 && idx < 20 && seatPtr[movieIndex][idx] == 1) 
    {
